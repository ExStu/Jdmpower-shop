      ...
      <Link href='http://ssr.jdmpower.ru/' className='hover:text-primary transition-colors duration-200 uppercase mr-auto' target='_blank'>SSR Rims</Link>
        <div className='flex flex-col '>
          <div className='flex items-center'>
            <FiPhoneCall size={19} className='mr-2'/><span className='mr-2'>Moscow:</span><a href={MoscowPhoneUrl}>{EnumPhones.MOSCOW}</a>
          </div>
          <div className='flex items-center'>
            <FiPhoneCall size={19} className='mr-2'/><span className='mr-2'>Krasnodar:</span><a href={KrasnodarPhoneUrl}>{EnumPhones.KRASNODAR}</a>
          </div>
        </div>
    </div>
  )
